...
export async function buildGenerationResponse(
  title: string,
  mode: Mode,
  rows: NormalizedRow[],
  ...
  const outputs = await renderAllModeOutputs(title, rows, denominatorMode);
  ...
  const archiveName = getZipName(title, mode);
...
export async function renderPreviewPng(payload: PreviewPayload) {
  ...
  const previewIndex = clampPreviewIndex(payload.previewIndex, rows.length);

  if (rows.length === 0) {
    return renderTitleBannerPng(payload.title);
  }

  const [titleBanner, textPanels] = await Promise.all([
    renderTitleBannerPng(payload.title),
    renderCumulativePanelPngs(rows),
  ]);
  const row = rows[previewIndex];
  const ooPanel = await renderOoPanelPng(row.numerator, row.denominator);
  return renderModeOnePanel(titleBanner, textPanels[previewIndex] ?? null, ooPanel);
}
...
async function renderModeOnePanel(
  ...
  const panels = await renderModeOnePanels(titleBanner, textPanel ? [textPanel] : [], ooPanel ? [ooPanel] : []);
  return panels[0];
}

async function renderModeOnePanels(titleBanner: Buffer, textPanels: Buffer[], ooPanels: Buffer[]) {
  ...
  for (let index = 0; index < panelCount; index += 1) {
    const currentTextPanel = textPanels[index];
    const currentOoPanel = ooPanels[index];
    ...
    if (currentTextPanel) {
      const resizedText = await sharp(currentTextPanel)
        .resize({ width: textWidth, fit: "inside" })
        .trim()
        .png()
        .toBuffer();
    ...
    if (currentOoPanel) {
      const resizedOo = await sharp(currentOoPanel)
        .resize({ width: ooWidth, fit: "inside" })
        .png()
        .toBuffer();
...
async function renderCumulativePanelPngs(rows: NormalizedRow[]) {
  ...
  const fullWidth = Math.max(...panelWidths.map((metadata) => metadata.width ?? 0), CANVAS_WIDTH) + OUTER_PADDING * 2;
  const fullHeight =
...
async function renderOoPanels(rows: NormalizedRow[]) {
  return Promise.all(rows.map(async (row) => renderOoPanelPng(row.numerator, row.denominator)));
}

async function renderTextPanelPng(text: string, symbols: NormalizedRow["symbols"], fontSize: number) {
...
async function renderTitleBannerPng(title: string) {
  ...
  const path = font.getPath(safeTitle, 0, 0, fontSize);
  ...
  const svg = `
    <svg width="${TITLE_BANNER_WIDTH}" height="${TITLE_BANNER_HEIGHT}" viewBox="0 0 ${TITLE_BANNER_WIDTH} ${TITLE_BANNER_HEIGHT}" xmlns="http://www.w3.org/2000/svg">
      ...
      <rect width="100%" height="100%" fill="${TITLE_RED}" />
...
async function renderSymbols(symbols: Array<Exclude<SymbolOption, "-">>, textHeight: number) {
  if (symbols.length === 0) {
    return "";
  }
